...
apps.each do |app|
  ...
  supervisor_service "firebolt" do
    command "java -Daws.accessKeyId=#{node[:firebolt][:aws_access_key_id]} -Daws.secretKey=#{node[:firebolt][:aws_secret_access_key]} -Dlogging.path=/home/#{node[:firebolt][:user]}/logs -Dsentry.dsn=#{node[:firebolt][:sentry_dsn]} -Dkannel.url=#{node[:firebolt][:kannel_url]} -Dkannel.port=#{node[:firebolt][:kannel_port]} -Dkannel.username=#{node[:firebolt][:kannel_username]} -Dkannel.password=#{node[:firebolt][:kannel_password]} d-Dadmin.username=#{node[:firebolt][:admin_username]} -Dadmin.password=#{node[:firebolt][:admin_password]} -Dspring.datasource.url=#{node[:firebolt][:database_url]} -Dspring.datasource.username=#{node[:firebolt][:database_username]} -Dspring.datasource.password=#{node[:firebolt][:database_password]} -jar firebolt.jar"
